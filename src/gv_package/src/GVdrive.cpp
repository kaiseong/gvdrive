...
namespace GVTerrain::robot
{
  ...
  void GVdrive::setControlMode(UserMode new_mode)
  {
    GVdrive::control_mode = new_mode;
  }

  void GVdrive::setConnectMode(ConnectMode new_mode)
  {
    GVdrive::connect_mode = new_mode;
  }
  ...
  bool GVdrive::startEthercat()
  {
    ...
    std::cout << "connect_mode : " << connect_mode << "  (0: No board, 1: RAIHUB)" << std::endl;
    std::cout << "ec_slavecount : " << ec_slavecount << std::endl;

    if (connect_mode == ConnectMode::NOBOARD)
    {
      start_cnt = 1;
    }
    else
    ...
    for (uint8_t i = start_cnt; i <= ec_slavecount; i++)
    {
    ...
    for (uint8_t i = start_cnt; i <= ec_slavecount; i++)
    {
      ...
      if (tx->ErrorCode != 0)
      {
        ...
        std::cerr << std::to_string(i) + ": fault code " + std::to_string((int)tx->ErrorCode) + " (binary: " + toBinaryString(tx->ErrorCode) + ") " + getErrorDescription(tx->ErrorCode) << std::endl;
      }
    }
    ...
    do
    {
      ...
      for (uint8_t i = start_cnt; i <= ec_slavecount; i++)
      {
        ...
        {
          std::cerr << std::to_string(i) + " (was: " + std::to_string(initialErrorCodes[i]) + ", binary: " + toBinaryString(initialErrorCodes[i]) + ")" << std::endl;
    ...
    if (faultClearCnt == -1)
    {
      ...
      for (uint8_t i = start_cnt; i <= ec_slavecount; i++) // here is 0819 test 4->3
      {
        auto tx = reinterpret_cast<robot::TxPDO *>(ec_slave[i].inputs);
        if (tx->ErrorCode != 0)
        {
    ...
    if (GVdrive::control_mode == CONTROL)
    {
      ec_send_overlap_processdata();
      int workCount = ec_receive_processdata(EC_TIMEOUTRET);

      if (workCount == (ec_slavecount - start_cnt + 1) * 3)
      {
  ...
  {
    for (int i = start_cnt; i <= ec_slavecount; i++)
    {
      auto rx = reinterpret_cast<robot::RxPDO *>(ec_slave[i].outputs);
